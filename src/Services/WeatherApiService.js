...
export default class WeatherApiService {
  ...
  _apiKey = "3ad9b2d2abb245dbb8e205619202010"; // hide
  ...
  getCurrentWeather = async (city_name) => {
    const data = await this.getResource(
      `current.json?key=${this._apiKey}&q=${city_name}&days=5`,
      "current_weather_weatherapi"
    );
  ...
  getForecastWeather = async (city_name) => {
    const data = await this.getResource(
      `forecast.json?key=${this._apiKey}&q=${city_name}&days=5`,
      "forecast_weather_weatherapi"
    );
  ...
  getHourlyWeather = async ({ city }) => {
    const data = await this.getResource(
      `forecast.json?key=${this._apiKey}&q=${city}&days=3`,
      "hourly_weather_weatherapi_" + city
    );
  ...
  _incrementApiCallCount = () => {
    ...
    console.warn(
      `WARNING: API CALL, YOU ONLY HAVE 1'000'000 of these. Count ${count}.`
    );
  };
}
